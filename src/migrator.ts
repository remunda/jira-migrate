...
export class JiraToShortcutMigrator {
  ...
  async migrateIssue(jiraKey: string, iterationId?: number): Promise<MigrationResult> {
    try {
  ...
  private async migrateAsEpic(jiraIssue: JiraIssue): Promise<MigrationResult> {
    ...
    const existing = await this.shortcutClient.findEpicByExternalId(jiraIssue.key);
    let result;
    
    if (existing) {
      console.log(`Updating existing epic ID ${existing.id} for ${jiraIssue.key}`);
  ...
  private async migrateAsStory(jiraIssue: JiraIssue, iterationId?: number): Promise<MigrationResult> {
    ...
    const mappedStateName = this.statusMapping[jiraIssue.fields.status.name] || 'Unstarted';
    let workflowState = await this.shortcutClient.findWorkflowStateByName(mappedStateName);
    ...
    const existing = await this.shortcutClient.findStoryByExternalId(jiraIssue.key);
    let result;
    
    if (existing) {
      console.log(`Updating existing story ID ${existing.id} for ${jiraIssue.key}`);
  ...
  private processADFContent(content: any[]): string {
    ...
    for (const node of content) {
      if (node.type === 'paragraph') {
        ...
        text += this.processListContent(node.content || [], node.type === 'orderedList') + '\n';
  ...
  async migrateBulk(jiraKeys: string[], iterationId?: number): Promise<MigrationResult[]> {
