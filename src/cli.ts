...
program
	...
	.option(
		"-i, --iteration <id>",
		"Assign to specific iteration by ID (Shortcut only)",
	)
	.option(
		"-c, --current-iteration",
		"Assign to current iteration (Shortcut only)",
	)
	...
	.action(async (jiraKey: string, options: MigrateOptions) => {
		try {
			...
			console.log(
				chalk.blue(`Target platform: ${targetPlatform.toUpperCase()}`),
			);
			...
			if (targetPlatform === "clickup") {
				...
				if (options.dryRun) {
					console.log(
						chalk.yellow(`[DRY RUN] Would migrate JIRA issue: ${jiraKey}`),
					);
					if (options.list) {
						console.log(
							chalk.yellow(`[DRY RUN] Would assign to list: ${options.list}`),
						);
					}
					if (options.parent) {
						console.log(
							chalk.yellow(
								`[DRY RUN] Would assign to parent task: ${options.parent}`,
							),
						);
					}
					return;
				...
				const result = await migrator.migrateIssue(
					jiraKey,
					options.list,
					options.parent,
					options.forceUpdate,
				...
				if (options.currentIteration) {
					...
					if (currentIter) {
						...
						console.log(
							chalk.blue(`Using current iteration: ${currentIter.name}`),
						);
					} else {
				...
				if (options.dryRun) {
					...
					if (iterationId) {
						console.log(
							chalk.yellow(
								`[DRY RUN] Would assign to iteration: ${iterationId}`,
							),
						);
					}
					return;
	...
	.action(async (options: BulkOptions) => {
		try {
			...
			if (options.keys) {
				...
				console.log(
					chalk.red("Please provide JIRA keys using --keys or --file option"),
				);
			...
			if (options.dryRun) {
				...
				if (targetPlatform === "shortcut" && options.iteration) {
					console.log(
						chalk.yellow(
							`[DRY RUN] Would assign to iteration: ${options.iteration}`,
						),
					);
				}
			...
			if (targetPlatform === "clickup") {
				...
				const results = await migrator.migrateBulk(
					jiraKeys,
					options.list,
					options.parent,
					options.forceUpdate,
				...
				successful.forEach((result) => {
					console.log(
						chalk.green(
							`  - ${result.jiraKey} → ${result.clickupUrl}${result.wasUpdate ? " (updated)" : ""}`,
						),
					);
				});
				...
				successful.forEach((result) => {
					console.log(
						chalk.green(`  - ${result.jiraKey} → ${result.shortcutUrl}`),
					);
				});
		...
		try {
			...
			if (issue) {
				...
				if (issue.fields.assignee) {
					console.log(
						chalk.white(`Assignee: ${issue.fields.assignee.displayName}`),
					);
				}
